...
\end{frame}
\note[itemize]{
	\item Note: this definition is the standard twisted case. In the literature, it is possible to find more general definitions corresponding to the abstract notion of Courant algebroid.
}
%-----------------------------------------------------------%

%-----------------------------------------------------------%
\subsection{Higher Courant Lie infinity algebras}
%-----------------------------------------------------------%
	...
	\only<2->{
		\tikz[overlay,remember picture]
		{
			\node[rounded corners,
            	 ...
            	 $e_i = \pair{X_i}{\alpha_i} \in \mathfrak{X} \oplus \Omega^{r} $ 
            	 \quad~,\qquad
            	 $f_i \in \bigoplus_{k=0}^{r-1}\Omega^k$.
            	 }}};
		}			
	}
...
\begin{frame}[t]{Details on the  \texorpdfstring{\cite{Zambon2012}} ~~construction}\label{frame:VinoDetails}
	...
	\begin{enumerate}[<+-| alert@+>]
		...
		\begin{tikzcd}[ampersand replacement=\&,column sep = large]
			Q= T^*[r]T[1]M \ar[r, two heads,] \& T[1]M \&[-2.5em] \\[-1.75em]
		...
		$\Rightarrow$   It lifts the \emph{de Rham} differential  to $C^\infty(Q)$ as $ \delta := \lbrace \mathcal{S},\cdot \rbrace$;
		\vfill

		\item Shifting the grading by $r$ we get $\mathcal{C}_r := C^\infty(Q)[r]$
		\\
		$\Rightarrow$   $(\mathcal{C}_r,\lbrace \cdot,\cdot\rbrace,\delta)$ is an honest \textbf{dg Lie algebra}.
...
\end{frame}
\note[itemize]{
	\item For the definition of $r$-Poisson algebra see \url{https://arxiv.org/pdf/1811.07395}.
	\item consider graded coordinates on $Q\sim (x^i,v_i, P_i, p_i)$ with
			$$ |x^i|=0,~ |v^i|=1,~ |P_i|=r, ~ |p_i|=r-1 ~;$$
	\item The degree $r+1$ function $\mathcal{S}= \sum_i v_i P_i$ encoding the de Rham differential satisfies
...
\begin{frame}{Higher Courant $L_\infty$-algebra as \textbf{Homotopy Fiber}}
	...
	%
	\begin{propblock}[${\Courant[\sigma]{r-1}}$ is a \emph{model for the homotopy fiber}] of the dgla embedding $\qquad \mathcal{C}_r^{\geq 0} \hookrightarrow \mathcal{C}_r$~.
	\end{propblock}
	\pause
	\begin{proofblock}
\begin{enumerate}[(1)]
...
\end{frame}
\note[itemize]{
	\item In Pridham's paper, the relevant results are Lemma 3.24, Proposition 4.42, and Corollary 4.57 (numbering from the latest arXiv version).
	\item 
}
%-----------------------------------------------------------%
