...
\begin{tikzcd}[column sep= small,row sep=small]
	...
	\ldots\ar["d",r] &
	\Omega^{n-2}(M) \ar["d",r] &
	\only<1>{\Omega^{n-1}_{\textrm{Ham}}(M,\sigma)}\only<2->{\phantom{\Omega^{n-1}_{\textrm{Ham}}(M,\sigma)} 
	\arrow[lll,black!60!green,to path={ node[rectangle,dashed,draw]{$\Omega^{n-1}_{\textrm{Ham}}(M,\sigma)$} -- +(0,-4ex) -| node[draw,ellipse,fill=white,scale=0.65,pos=0.25]{$\{\cdot,\ldots,\cdot\}_k$}(\tikztotarget)node[rectangle,dashed,draw]{$\Omega^{n+1-k}(M)$}}]}
	&
	\\
\end{tikzcd}
%\end{standaloneframe}
