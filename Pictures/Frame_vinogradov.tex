...
\begin{minipage}[t]{\textwidth}
				 ...
				 \tikz[baseline]{
					...
					,
					 \tikz[baseline]{
				            ...
				            {$ \lbrack \cdot,\cdot \rbrack_\sigma$};
					}					
				\big)
			\end{displaymath}	
	\end{minipage}
	\begin{minipage}[t]{0.6\textwidth}
		...
		\tikz[na] \node[scale=0.5,coordinate,fill=blue!20,draw,circle] (n1) {};		    
		   		 \onslide<1->{Vector bun. $E^{(r)}=TM\oplus (\wedge^{r}T^\ast M)$;}
		\\
	...
	\begin{minipage}[t]{\textwidth}
		...
		\onslide<3->{pairing
			\begin{tikzcd}[baseline=(O.base),ampersand replacement=\&]
				|[alias=O]|\pairing_{\pm}~:\&[-4em] E^{(r)}\otimes E^{(r)} \ar[r]\& \wedge^{r-1}T^\ast M
				\\[-2em]
		...
		\onslide<4->{Courant bracket (twisted by $\sigma\in \Omega^{r+1}_{\text{cl}}$)
			\begin{displaymath}
				\begin{tikzcd}[ampersand replacement=\&]
					\lbrack\cdot,\cdot\rbrack_\sigma~:\&[-3em] \Gamma(E^{(r)})\otimes \Gamma(E^{(r)}) \ar[r]\& \Gamma(E^{(r)})%\Omega^{n-2}(M)
					\\[-2em]
					\& \binom{x_1}{\alpha_1}\otimes\binom{x_2}{\alpha_2} \ar[r,mapsto]				
					\& \binom{[x_1,x_2]}{
						\mathcal{L}_{x_1}\alpha_2 - \mathcal{L}_{x_2}\alpha_1 - 
						d\left\langle \binom{x_1}{\alpha_1},\binom{x_2}{\alpha_2} \right \rangle_- +
						\iota_{x_1}\iota_{x_2} \sigma
					}
					~.
				\end{tikzcd}		
			\end{displaymath}
