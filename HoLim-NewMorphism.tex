...
%-----------------------------------------------------------%
\begin{frame}{The degree-one case}
	%
	\begin{block}{Goal:}
	...
	\begin{displaymath}
		...
		\end{tikzcd}
	\end{displaymath}
	\cite{Zambon2012} proved that the vertical arrows are the linear part of an $L_\infty$-morphism $\Courant[\sigma]{0} \to \Courant{1}$.
	\end{exblock}
	\vfill
...
%-----------------------------------------------------------%
\begin{frame}{The canonical chain map}
	%
	...
	\begin{thmblock}[{[Fiorenza--Miti 2026, \cite{Fiorenza2026}]}]
...
%-----------------------------------------------------------%
\begin{frame}{Homotopy fibers and naturality}
	%
...
%-----------------------------------------------------------%
\begin{frame}{The Cartan dg-Lie algebra}
  Inside $\mathcal{C}_r$ there is a more manageable dg-Lie subalgebra with the same negative part.
  \vfill \pause
...
%-----------------------------------------------------------%
\begin{frame}{The small dg-Lie model}
	\small
	  \begin{defblock}[The \emph{small} dgla]
		...
		Observe that any $\sigma \in \Omega^{r+1}_{\mathrm{cl}}$ is a Maurer--Cartan element of degree $1$ in $\g_r$.
		\pause
		\smallskip
...
\note[itemize]
{
	\item Recall $\Der(\Omega)$ clearly acts on $\Omega[r]$ as derivations ($\Omega$ is an algebra w.r.t. $\wedge$). In particular this is true for $\Cartan$.
	\item Being a $\Cartan$ module means to be equipped with such an action.
	\item A general approach to module is to define them as an algebra structure on the direct sum $\Omega[r] \oplus \Cartan$ such that the product of an elment in $\Cartan$ with an element in $\Omega$ is the module action and the action of $\Omega$ on $\Omega$ is trivial. this is what we mean with by semidirect product $\ltimes$. (see remark 2.13 in \url{https://arxiv.org/pdf/2506.00234}).
}
%-----------------------------------------------------------%

%-----------------------------------------------------------%
\begin{frame}{The reduced homotopy-fiber model}
	\begin{upshotblock}
	  Replace $\mathcal{C}_{r,\sigma}, \mathcal{C}_{r+1}$ with $\g_{r,\sigma}, \g_{r+1}$ in the previous commutative square:
		\begin{displaymath}
			\begin{tikzcd}[ampersand replacement=\&, column sep = large]
	...
	The construction must in particular work in the untwisted case $\sigma=0$.
\end{frame}
\note[itemize]
{
	\item .
...
%-----------------------------------------------------------%
\begin{frame}{The untwisted square}
	...
	\begin{itemize}[<+-|@alert@+>]
		\item $\Phi_1:\g_{r}\to \g_{r+1}$ is a morphism of cochain complexes:
		\begin{displaymath}
		\resizebox{\textwidth}{!}{$
		\begin{tikzcd}[ampersand replacement=\&, column sep = small]
			0\ar[r]\ar[d] \&
...
%-----------------------------------------------------------%
\begin{frame}{The quadratic correction}
...
%-----------------------------------------------------------%
\begin{frame}{Twisting by a Maurer--Cartan element}
  Let $\sigma \in \Omega^{r+1}_{\mathrm{cl}}$. Use $\sigma$ to twist the previous square:
  \vfill \pause	
	...
	\only<4->{
	\blfootnote{
		One has to check that $\Phi_\sigma$ preserves the positive part. This is immediate since $(\Phi_\sigma)_2 = \Phi_2$.\\
		Moreover, we have 
...
%-----------------------------------------------------------%
\begin{frame}{The canonical morphism}
  \textcolor{gray}{Let $\sigma \in \Omega^{r+1}_{\mathrm{cl}}$ and twist the entire square.}
  \vfill
...
%-----------------------------------------------------------%
\begin{frame}{Multisymplectic observables}
	Recall:
	\begin{displaymath}
		\g_{r,\sigma}^{<0} := \left(
		\begin{tikzcd}[ampersand replacement=\&]
...
%-----------------------------------------------------------%
\begin{frame}{The observable morphism as a homotopy-fiber map}
	\begin{thmblock}[{[Fiorenza--Miti 2026, \cite{Fiorenza2026}]}]
	...
	\begin{displaymath}
		\begin{tikzcd}[ampersand replacement=\&]
			...
			\g_{r,\sigma}^{~\geq 0} \ar[r,hook] \ar[ur,Rightarrow]\&
			\g_{r,\sigma}
		\end{tikzcd}
	\end{displaymath}
	\end{thmblock}
