...
\end{frame}
\note[itemize]{
	\item Lie algebras are $L_\infty$-algebras concentrated in degree 0!
}%------------------------------------------------------------%	
...
\begin{frame}{2-plectic case}
	...
	\begin{defblock}[Hamiltonian pairs \emph{(same as before!)}]
		\begin{displaymath}
			\Ham^{(1)}_\sigma = \{(X,\alpha) \mid \iota_X \sigma + \d \alpha = 0 \}
...
%-----------------------------------------------------------%
\begin{frame}{2-plectic case: algebraic structures}
	\begin{itemize}
...
\begin{frame}{Going higher!}
	%
	Fix a closed $(r$+$2)$-form $\sigma \in \Omega^{r+2}_{\mathrm{cl}}$~;
	\vfill\pause
	...
	\begin{defblock}[Hamiltonian pairs \emph{(same as before!)}]
		...
		\begin{displaymath}
			\Ham^{(r)}_\sigma = 
		\{ (X,\alpha) \mid \iota_X \sigma + \d \alpha  = 0 \}
		\subseteq \X\oplus \Omega^{r} = \Gamma(E^{(r)})
		\end{displaymath}
	\end{defblock}
	\vfill\pause
...
\begin{frame}{Going higher!~Algebraic structures}
	...
	\begin{propblock}
		...
		\begin{tikzcd}[ampersand replacement=\&,column sep = small]
			\& \text{\small (deg $-r$)} \& \cdots \& \&\text{\small (deg $-1$)} \& \text{\small (deg $0$)} \\[-.5em]
			\Rogers[\sigma]{r}   \ar[r,phantom,":="]
			\&
			...
			\cdots
			\&
			\Omega^{r-1} \ar[r,"{(0,\d)}"] \ar[d,"id"]
			\& \Ham_{\sigma}^{(r)} \ar[d,hook]
			\\
			\Courant[\sigma]{r} \ar[r,phantom,":="]
			\&
			\Omega^0 \ar[r,"{\d}"]
			\&
			\Omega^1 \ar[r,"{\d}"] 
	...
	\begin{itemize}
		\item \cite{Rogers2010} endows $\Rogers[\sigma]{r}$ with a $L_\infty$-algebra structure.\pause
		\item \cite{Zambon2012} endows $\Courant[\sigma]{r}$ with a $L_\infty$-algebra structure.\pause
		...
		\item \cite{Miti2024} shows that the vertical arrows are the linear part of a $L_\infty$-morphism
			$$\Rogers[\sigma]{r} \to \Courant[\sigma]{r}~.$$
	\end{itemize}
...
\end{frame}
\note[itemize]{
	\item Observe that in principle one can go on
	$$\Rogers[\sigma]{r}\to \Courant[\sigma]{r} \to \Courant{r+1} \to \Courant{r+2} \to \dots $$
	However, we will se that the thre rightomost arrows are "essentially" given by de Rham derivative in the linear part, hence they compose to $0$. More precisely, the image of the composition is $\Cartan$. see below.
}
%-----------------------------------------------------------%
