...
\begin{frame}{(Pre)-symplectic Case: algebraic structures (1)}
	\begin{lemblock}[$\Ham^{(0)}_\sigma$ carries a Lie algebra structure:]
		...
		\end{equation}
	\blfootnote{rmk: $\sigma(X,Y)$ is an Hamiltonian 1-form for $[X,Y]$!}
	\end{lemblock}
	\vfill
...
\begin{frame}[fragile]{2-plectic case: algebraic structures}
	\begin{itemize}
		\item \emph{do $\Ham^{(1)}_\sigma$ and $E^{(1)}$ carry a Lie algebra structure?}
			\begin{itemize}
				\item[$\bullet$] $\Gamma(E^{(1)})$ has a canonical antisymmetric bracket (called \emph{Courant bracket});
				\item[$\bullet$] Same for $\Ham^{(1)}_\sigma$ (generalizing \eqref{eq:Bracket-HamPairs} to 1-forms)
			\end{itemize}
		\item They do \underline{not} satisfy the Jacobi identity on the nose but only \emph{"up to homotopy"}. $\Rightarrow$ there could be a $L_\infty$-algebra lying around.
	\end{itemize}	
	\vfill
	...
	\cite{Rogers2013}: Vertical arrows are the linear part of a $L_\infty$-morphism $$\Rogers[\sigma]{1} \to \Courant[\sigma]{1}~.$$
	\end{propblock}
\end{frame}
%-----------------------------------------------------------%
...
\begin{frame}{Going higher!~Algebraic structures}
	...
	\begin{itemize}
		...
		\item \cite{Miti2021} shows that the vertical arrows are the linear part of a $L_\infty$-morphism
...
\begin{frame}{Goals}
	\begin{itemize}
		\item \cite{Miti2021} explicitly contructs a morphism of $L_\infty$-algebras
		\begin{displaymath}
			\begin{tikzcd}[ampersand replacement=\&]
	...
	\begin{claimblock}
		\centering
		We rely on an algebraic framework that explains the \emph{existence} and \emph{\underline{naturality}} of the morphism introduced in \cite{Miti}.
	\end{claimblock}
	\vfill
